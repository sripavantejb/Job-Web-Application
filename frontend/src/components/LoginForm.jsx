...
const SignInForm = React.memo(
  ({ email, password, onEmailChange, onPasswordChange, handleSignin }) => (
    <form
      onSubmit={handleSignin}
      className="bg-white flex flex-col items-center justify-center p-12 text-center h-full"
    >
      ...
      />
      <a href="#" className="text-xs my-3">
        Forgot your password?
      </a>
      <button
        type="submit"
        className="bg-blue-500 hover:bg-blue-600 text-white font-bold uppercase text-xs py-3 px-11 rounded-full transition-colors"
      >
        Sign In
      </button>
    </form>
...
const SignUpForm = React.memo(
  ({
    ...
    >
      ...
      <button
        type="submit"
        className="bg-blue-500 hover:bg-blue-600 text-white font-bold uppercase text-xs py-3 px-11 rounded-full mt-4 transition-colors"
      >
        Sign Up
      </button>
    </form>
...
const LoginForm = () => {
  ...
  const handleSignin = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://job-web-application-ktk3.onrender.com/api/auth/login",
        options
      );
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://job-web-application-ktk3.onrender.com/api/auth/register",
        options
      );
      const data = await response.json();
            ...
            {showSignUp ? (
              <>
                ...
                <button
                  ...
                  className="bg-transparent border-2 border-white font-bold uppercase text-xs py-3 px-11 rounded-full"
                >
                  Sign In
                </button>
              </>
